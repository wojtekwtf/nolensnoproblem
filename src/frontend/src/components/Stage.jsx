  ...
  return (
    <li key={1} className='pb-10 relative'>
      {props.currentStep > props.position ? (
        <>
          {!props.isLast ? (
            <div className="absolute left-4 top-4 -ml-px mt-0.5 h-full w-0.5 bg-gray-900" aria-hidden="true" />
          ) : null}
        ...
        <>
          ...
          <div className="group relative flex items-start hover:cursor-pointer" aria-current="step">
            ...
            <span className="ml-4 flex min-w-0 flex-col">
              ...
              <span className="text-sm text-gray-500 mb-2">{props.description}</span>
              {props.component}
            </span>
          </div>
        </>
